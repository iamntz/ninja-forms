...
class NF_AJAX_Controllers_Form extends NF_Abstracts_Controller
{
    ...
    public function save()
    {
        ...
        $form = $this->get_form( $form_data );

        $form->update_settings( $form_data[ 'settings' ] )->save();

        if( isset( $form_data[ 'fields' ] ) ) {
            $this->parse_form_fields( $form_data );
        }

        if( isset( $form_data[ 'deleted_fields' ] ) ){
            $this->parse_deleted_fields( $form_data );
        }

        if( isset( $form_data[ 'actions' ] ) ) {
            $this->parse_actions( $form_data );
        }

        $this->parse_actions2( $form_data );

        if( isset( $form_data[ 'deleted_actions' ] ) ){
            $this->parse_deleted_actions( $form_data );
        }
    ...
    protected function get_form( $form_data )
    {
    ...
    protected function parse_form_fields( $form_data )
    {
    ...
    protected function parse_deleted_fields( $form_data )
    {
     ...
     */
    protected function parse_actions( $form_data )
    {
     ...
     */
    protected function parse_actions2( $form_data )
    {
        foreach ($form_data['actions'] as $action_data) {
    ...
    protected function parse_deleted_actions( $form_data )
    {
